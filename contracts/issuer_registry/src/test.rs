...
fn setup() -> (Env, BenzoIssuerRegistryClient<'static>, Address) {
    ...
    (
        env.clone(),
        BenzoIssuerRegistryClient::new(&env, &id),
        admin,
    )
}
...
fn registers_issuer_and_advances_root() {
    ...
    assert_ne!(
        reg.current_root(),
        root0,
        "registering must advance the root"
    );
...
fn rejects_duplicate_issuer() {
    ...
    assert!(
        res.is_err(),
        "a second registration of the same issuer must fail"
    );
}
...
fn unknown_root_is_rejected() {
    let (env, reg, _admin) = setup();
    assert!(
        !reg.is_known_root(&U256::from_u32(&env, 1)),
        "bogus root must be unknown"
    );
    assert!(
        !reg.is_known_root(&U256::from_u32(&env, 0)),
        "zero is never a valid root"
    );
}
