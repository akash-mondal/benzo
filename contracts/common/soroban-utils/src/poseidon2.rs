...
#[cfg(test)]
mod host_differential_tests {
    //! Direct host-vs-guest Poseidon2 differential. This is the pre-ceremony gate
    //! that keeps the circuit, TS mirror, and Soroban host byte-identical.
    //!
    //! The known-answer vectors below are generated by the TS Poseidon2 mirror
    //! (packages/core/src/crypto/poseidon2.ts), which is byte-identical to the
    //! circom templates. If the CAP-0075 host parameterization ever drifts from
    //! the circuit/SDK params, these equalities break — catching it BEFORE the
    //! trusted setup rather than after (where a drift would silently reject every
    //! honest proof on-chain). The host computes t=3 (`poseidon2_hash2`) and t=2
    //! (`poseidon2_compress`); the t=4 note commitment is circuit/SDK-only and is
    //! enforced by the green circuit proving tests.

    use super::{poseidon2_compress, poseidon2_hash2};
    use soroban_sdk::{Bytes, Env, U256};

    fn expect(env: &Env, be: &[u8; 32]) -> U256 {
        U256::from_be_bytes(env, &Bytes::from_array(env, be))
    }

    #[test]
    fn host_hash2_t3_matches_ts_mirror() {
        let env = Env::default();
        let h = |a: u32, b: u32, sep: Option<u32>| {
            poseidon2_hash2(
                &env,
                U256::from_u32(&env, a),
                U256::from_u32(&env, b),
                sep.map(|s| U256::from_u32(&env, s)),
            )
        };
        // hash2(1, 2, sep=0x01) — ASP-leaf-style domain
        assert_eq!(
            h(1, 2, Some(1)),
            expect(
                &env,
                &[
                    44, 80, 198, 230, 66, 213, 199, 200, 179, 89, 71, 165, 240, 14, 19, 145, 220,
                    68, 59, 23, 183, 187, 109, 197, 214, 188, 25, 53, 11, 109, 252, 180,
                ]
            )
        );
        // hash2(3, 4, sep=0x02) — nullifier-style domain
        assert_eq!(
            h(3, 4, Some(2)),
            expect(
                &env,
                &[
                    47, 167, 245, 102, 20, 163, 200, 72, 203, 247, 45, 112, 74, 211, 142, 234, 177,
                    2, 139, 14, 119, 219, 125, 116, 137, 44, 138, 90, 114, 202, 108, 180,
                ]
            )
        );
        // hash2(5, 6, sep=None) — None maps to domain 0
        assert_eq!(
            h(5, 6, None),
            expect(
                &env,
                &[
                    8, 87, 223, 114, 214, 237, 119, 82, 203, 146, 139, 68, 162, 236, 68, 26, 32,
                    228, 25, 39, 92, 115, 98, 214, 17, 109, 58, 250, 198, 62, 187, 9,
                ]
            )
        );
    }
    ...
    #[test]
    fn host_compress_t2_matches_ts_mirror() {
        let env = Env::default();
        // compress(7, 8) — the Merkle-node compression the tree contract uses.
        assert_eq!(
            poseidon2_compress(&env, U256::from_u32(&env, 7), U256::from_u32(&env, 8)),
            expect(
                &env,
                &[
                    3, 18, 251, 99, 126, 92, 89, 27, 68, 166, 185, 99, 94, 203, 126, 16, 76, 123,
                    137, 191, 42, 146, 224, 76, 99, 182, 220, 180, 121, 189, 130, 251,
                ]
            )
        );
    }
