...
/**
 * Extension surface adapter (IBenzoPlatform).
 * In-memory stubs are used here so the package builds; the real bindings
 * (see README) swap storage/keychain/clipboard for the extension runtime.
 */
export class ExtensionPlatform implements IBenzoPlatform {
  ...
  async openLink(url: string): Promise<void> { void url; /* TODO: extension navigation */ }
}
