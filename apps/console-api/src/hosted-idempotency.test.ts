...
vi.mock("./google-oidc.js", () => ({
  ...
  verifyGoogleIdToken: vi.fn(async () => ({
    sub: "user-console-idem",
    iss: "https://accounts.google.com",
    aud: "google-client",
    email: "console-idem@example.com",
    name: "Console Idem",
    exp: Math.floor(Date.now() / 1000) + 3600,
  })),
}));

const ENV_KEYS = ["VERCEL", "GOOGLE_CLIENT_ID", "BENZO_ACCOUNT_SALT", "BENZO_PRIVATE_EVENT_SECRET"] as const;
...
test("hosted console writes require an idempotency key after auth and before tenant storage", async () => {
  ...
  const res = await request(handle, `/api/rpc?path=${encodeURIComponent("/payments")}`, {
    method: "POST",
    headers: { authorization: "Bearer verified-google-token", "content-type": "application/json" },
    body: JSON.stringify({ amount: { amount: "10000000", assetCode: "USDC" }, toCounterpartyId: "cp_1" }),
  });

  expect(res.status).toBe(428);
  await expect(res.json()).resolves.toMatchObject({ error: "Idempotency-Key header is required for hosted console writes." });
});
