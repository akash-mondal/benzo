...
function fingerprintAccount(account: BenzoAccount): string {
  return createHash("sha256")
    .update(`console|${account.stellarAddress ?? ""}|${account.spendPub.toString()}|${account.mvkScalar.toString()}`)
    .digest("hex")
    .slice(0, 32);
}
...
export async function authFromRequest(req: IncomingMessage): Promise<AuthContext | null> {
  ...
  const key = createHash("sha256").update(`console|${claims.iss}|${claims.aud}|${claims.sub}`).digest("hex").slice(0, 32);
  const account = accountFromOidc(
    { iss: claims.iss, aud: claims.aud, sub: claims.sub },
    { app: "business", salt: accountSalt() },
  );
  account.label = `console-${key.slice(0, 8)}`;
