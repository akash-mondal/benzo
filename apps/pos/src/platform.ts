...
/**
 * Pos surface adapter (IBenzoPlatform).
 * In-memory stubs are used here so the package builds; the real bindings
 * (see README) swap storage/keychain/clipboard for the pos runtime.
 */
export class PosPlatform implements IBenzoPlatform {
  readonly name = "pos";
  ...
  async openLink(url: string): Promise<void> { void url; /* TODO: pos navigation */ }
}
