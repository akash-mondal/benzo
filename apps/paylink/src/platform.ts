...
/**
 * Paylink surface adapter (IBenzoPlatform).
 * In-memory stubs are used here so the package builds; the real bindings
 * (see README) swap storage/keychain/clipboard for the paylink runtime.
 */
export class PaylinkPlatform implements IBenzoPlatform {
  readonly name = "paylink";
  ...
  async openLink(url: string): Promise<void> { void url; /* TODO: paylink navigation */ }
}
